...
public class Workspace extends ViewGroup {
	...
	/*
	 * private Launcher mLauncher; private DragController mDragger;
	 */
	...
	public Workspace(Context context, LinearLayout mTitle) {
		super(context);
		initWorkspace();
		this.mTitle = mTitle;
	}
	...
	public void addView(View child, int index, LayoutParams params) {
		/*
		 * if (!(child instanceof CellLayout)) { throw new
	...
	public void computeScroll() {
		...
		if (mScroller.computeScrollOffset()) {
			...
			if (listener != null) {
				for (SnapToScreenListener listener : this.listener) {
					listener.snapToScreen(mNextScreen);
				}
			}
			mCurrentScreen = Math.max(0,
	...
	private void snapToDestination() {
		...
		final int whichScreen = (this.getScrollX() + (screenWidth / 2))
				/ screenWidth;
	...
	public void snapToScreen(int whichScreen) {
		...
		mScroller.startScroll(this.getScrollX(), 0, delta, 0, Math.abs(delta));
		invalidate();

	}

	private List<SnapToScreenListener> listener;
	public void addSnapToScreenListener(SnapToScreenListener listener) {
		if(this.listener == null){
			this.listener = new ArrayList<SnapToScreenListener>();
		}
